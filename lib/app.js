...
function createApp ({ engine, ext, staticPrefix, dataPrefix = 'data' }) {
  ...
  app.get('/', (req, res, next) => {
    readJSON(join(dataPrefix, 'website.json'))
      .then(w => res.redirect('/' + w.defaultLanguage), next)
  })
  app.get('/:lang/:page?', (req, res, next) => {
    if (req.params.page === 'favicon.ico') return next()
    const page = req.params.page || 'index'
    const p = join(app.get('views'), `pages/${page}.${ext}`)
    if (!existsSync(p)) return res.redirect(`/${req.params.lang}/404`)
    res.render(`pages/${page}`, { signature: ['page', page] })
  })
  app.get('/:lang/:type/:id', (req, res) => {
